...
function adaptarAnime(nodo) {
  ...
  return {
    ...
    // MAL mezcla generos y demografias en la misma lista (Shounen, Seinen
    // conviven con Action o Fantasy). Se conserva tal cual: para el grafo de
    // generos esa mezcla enriquece, no estorba.
    genres: (nodo.genres || []).map(g => ({ mal_id: g.id, type: 'anime', name: g.name || '' })),
